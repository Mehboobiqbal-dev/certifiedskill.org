...
async function handler(req, res) {
    ...
    try {
        ...
        doc.rect(30, 30, doc.page.width - 60, doc.page.height - 60).lineWidth(4).stroke('#003366');
        doc.fillColor('#003366').fontSize(48).text('Certificate of Achievement', {
            align: 'center',
            underline: true
        });
        doc.moveDown(1.5);
        ...
        doc.moveDown(0.5);
        doc.fontSize(40).text(certificate.userName, {
            align: 'center'
        });
        doc.moveDown(1);
        ...
        doc.moveDown(0.5);
        doc.fontSize(36).text(certificate.examName, {
            align: 'center'
        });
        doc.moveDown(1.5);
        doc.fontSize(20).text(`Issued on: ${new Date(certificate.issuedAt).toLocaleDateString()}`, {
            align: 'center'
        }).moveDown(0.5).text(`Certificate Number: ${certificate.certificateNumber}`, {
            align: 'center'
        });
        doc.end();
    } catch (error) {
